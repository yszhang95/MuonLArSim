...
G4VPhysicalVolume* DetectorConstruction::Construct() {
    ...
    for (size_t i = 0; i < boundaries.size(); ++i) {
        ...
        G4double x_center = (b[0][0] + b[0][1]) / 2.0 * cm;
        G4double y_center = (b[1][0] + b[1][1]) / 2.0 * cm;
        G4double z_center = (b[2][0] + b[2][1]) / 2.0 * cm;

        G4double x_half = std::abs(b[0][1] - b[0][0]) / 2.0 * cm;
        G4double y_half = std::abs(b[1][1] - b[1][0]) / 2.0 * cm;
        G4double z_half = std::abs(b[2][1] - b[2][0]) / 2.0 * cm;
        ...
        logic->SetUserLimits(new G4UserLimits(maxStep));
